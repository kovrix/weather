...
export class OpenMeteoAdapter implements WeatherProvider {
  ...
  async getWeather(params: { location: string }) {
    ...
    const locationName = [match.name, match.country].filter(Boolean).join(", ");
    const forecastUrl = `https://api.open-meteo.com/v1/forecast?latitude=${match.latitude}&longitude=${match.longitude}&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&daily=weather_code,temperature_2m_max,temperature_2m_min&wind_speed_unit=kmh&temperature_unit=celsius&timezone=auto`;
